...
import cherokee.conjugation.verbal.stemming.PatternMatchReplacement;
...
public class AffixSplitterRewrite extends Thread {
    ...
    private static final def f = new File("affixSplitterRewrite.txt")
...
//    }
    ...
    public Verb processLine(String line) {
        Verb verb = new Verb();

        line = simpleAffixSplits(verb, line);
    ...
    private static List<PatternMatchReplacement> getSuffixMatches() {
        ...
        p.regexmatch = "[" + StringUtils.join(c_v) + "]Ꮎ";
        String match = "";
        String replace = "";
        for (String c : c_v) {
            match += "==" + c + "Ꮎ,";
...
//		addCommonPrepronounPermutations(matches, p);
    ...
    private static PatternMatchReplacement prefixWithDe(PatternMatchReplacement p) {
        ...
        q.regexmatch = prefixWithDe(p.regexmatch);
...
//			q.replacement[i]="Ꮥ= "+q.replacement[i];
    ...
    private static PatternMatchReplacement prefixWithNi(PatternMatchReplacement p) {
        ...
        q.regexmatch = prefixWithNi(p.regexmatch);
...
//			q.replacement[i]="Ꮒ= "+q.replacement[i];
    ...
    private static PatternMatchReplacement prefixWithYi(PatternMatchReplacement p) {
        ...
        q.regexmatch = prefixWithYi(p.regexmatch);
...
//			q.replacement[i]="Ᏹ= "+q.replacement[i];
    ...
    private String doYiPrefixSplit(Verb verb, String line) {
        line = line.replaceAll(yi_prefix, yi_prefixReplace);
        if (line.contains("==")) {
            verb.initialPrefix.yi = true
    ...
    private String doWiPrefixSplit(Verb verb, String line) {
        line = line.replaceAll(wi_prefix, wi_prefixReplace);
        if (line.contains("==")) {
            verb.initialPrefix.wi = true
    ...
    private String simpleAffixSplits(Verb verb, String line) {
        line = doYiPrefixSplit(verb, line);
        line = doWiPrefixSplit(verb, line);
        println verb.initialPrefix.yi
